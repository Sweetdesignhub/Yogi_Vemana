...
const useAvatar = ({
  ...
  const speakNext = useCallback(
    (text, endingSilenceMs = 0, skipUpdatingChatHistory = false) => {
      return new Promise((resolve, reject) => {
        ...
        console.log(
          "speakNext called for:",
          text,
          "isSpeaking:",
          isSpeaking,
          "avatarSynthesizer state:",
          avatarSynthesizer.current ? "initialized" : "null"
        );
        setIsSpeaking(true);
        ...
        avatarSynthesizer.current
          ...
          .then((result) => {
            if (
              ...
              console.error(
                `Error speaking SSML. Result ID: ${result.resultId}`
              );
          ...
          .catch((error) => {
            ...
            if (spokenTextQueue.current.length > 0) {
              const nextText = spokenTextQueue.current.shift();
              console.log(
                "Processing next in spokenTextQueue after error:",
                nextText
              );
              speakNext(nextText).then(resolve).catch(reject);
            } else {
  ...
  const stopSpeaking = useCallback(() => {
    console.log(
      "Stop speaking clicked, isSpeaking:",
      isSpeaking,
      "avatarSynthesizer state:",
      avatarSynthesizer.current ? "initialized" : "null"
    );
  ...
  const processResponseQueue = useCallback(async () => {
    console.log(
      "processResponseQueue called, queue:",
      responseQueue.current,
      "isProcessingQueue:",
    ...
    if (isProcessingQueue.current || responseQueue.current.length === 0) {
      console.log(
        "processResponseQueue skipped due to isProcessingQueue or empty queue"
      );
      return;
    }
    ...
    console.log(
      "isProcessingQueue set to false, remaining queue:",
      responseQueue.current
    );
  ...
  const connectAvatar = useCallback(
    (retryCallback = () => {}, initialQuery = null) => {
      ...
      if (speechConfig.apiKey === "") {
        ...
        showErrorPopup("Please try again later");
      ...
      if (
        ...
        console.error(
          "speechConfig.privateEndpoint is empty when enablePrivateEndpoint is true"
        );
        showErrorPopup("Please try again later");
        retryCount.current = 0;
        setTimeout(() => window.location.assign("/"), 5000);
      ...
      avatarSynthesizer.current.avatarEventReceived = (s, e) => {
        console.log(
          `Avatar event received: ${e.description}, offset: ${
            e.offset / 10000
          }ms`
        );
      };
  ...
  const setupWebRTC = useCallback(
    (
      ...
      console.log(
        "Setting up WebRTC with ICE server:",
        iceServerUrl,
        "initialQuery:",
        initialQuery
      );
      ...
      peerConnection.current.ontrack = (event) => {
        console.log("🎵 ONTRACK EVENT:", event.track.kind, event);

        const remoteVideoDiv = document.getElementById("remoteVideo");
        if (!remoteVideoDiv) {
          console.error("❌ remoteVideo div not found in DOM!");
        ...
        if (event.track.kind === "audio") {
          ...
          if (oldAudio) {
            console.log("🗑️ Removing old audio element");
            oldAudio.remove();
          }
          ...
          audioElement.onpause = () =>
            console.log("⏸️ Audio ONPAUSE event fired");
          ...
          if (oldVideo) {
            console.log("🗑️ Removing old video element");
            oldVideo.remove();
          }
          ...
          videoElement.style.width = "450px";
          videoElement.style.height = "300px";
          videoElement.style.display = "none"; // Hide video since we're using custom image
          remoteVideoDiv.appendChild(videoElement);

          console.log("✅ Video element added (hidden)");
        }
      };
      ...
      peerConnection.current.oniceconnectionstatechange = () => {
        ...
        if (peerConnection.current.iceConnectionState === "failed") {
          ...
          if (retryCount.current >= maxRetries) {
            ...
            retryCount.current = 0;
            console.error(
              `WebRTC connection failed after ${maxRetries} attempts`
            );
            setTimeout(() => window.location.assign("/"), 5000);
          } else {
            setTimeout(() => {
          ...
          if (r.reason === SpeechSDK.ResultReason.SynthesizingAudioCompleted) {
            ...
            if (!initialQuery && !hasWelcomed.current) {
              const welcomeMessage =
                "Hi, I am Yogi Vemana. How can I help you today?";
  ...
  const startSession = useCallback(
    (initialQuery = null) => {
      ...
      if (useLocalVideoForIdle) {
        ...
        if (initialQuery) {
          console.log(
            "Triggering initial query for local video:",
            initialQuery
          );
          handleUserQuery(initialQuery, initialQuery, "");
        }
